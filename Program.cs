...
internal static class Program
{
    ...
    private static bool WriteFileWithPolicy(string path, byte[] data, int overwriteMode)
    {
        ...
        switch (overwriteMode)
        {
            ...
            default:
                throw new ArgumentOutOfRangeException(nameof(overwriteMode),
                    "Expected 0 (never), 1 (ask), or 2 (always).");
        }
    }
    ...
    private static int CmdExportJson(string[] args)
    {
        ...
        if (args.Length < 1)
            return Fail(
                "Usage: rivaxtract export-json <archive> --out structure.json [--pretty] [--include-modmap-raw]");

        var archive = args[0];
        var outJson = "";
        var pretty = false;
        var includeModMapRaw = false;
        ...
        uint modmapSize = 0;
        for (var mi = 0; mi < dsa.mModuleCount; mi++)
        ...
        for (var i = 0; i < dsa.Count; i++)
        {
            ...
            root.Entries!.Add(new JsonEntry
            {
                ...
                Name = e.Name ?? "",
                RawFilenameHex = BytesToHex(e.NameRaw13 ?? []),
        ...
        for (var mi = 0; mi < dsa.mModuleCount; mi++)
        {
            ...
            root.Modules!.Add(new JsonModule
            {
                ...
                Name = m.Name ?? "",
                RawFilenameHex = BytesToHex(m.NameRaw14 ?? []),
                SizeSlotsU32 = m.Size,
        ...
        if (includeModMapRaw)
        {
            ...
            for (var i = 0; i < modmapSize; i++) raw.Add((ushort)dsa.mModMap[i]);
        ...
        var opts = new JsonSerializerOptions
        {
            ...
            TypeInfoResolver = RivaJsonContext.Default
        };
    ...
    private static int CmdExportViews(string[] args)
    {
        ...
        if (args.Length < 1)
            return Fail(
                "Usage: rivaxtract export-views <archive> --out <root> [--views ...] [--mode hard|symlink|copy] [--dummy omit|marker] [--overwrite ask|never|always]");

        var archive = args[0];
        var outRoot = "";
        var viewsArg = "index,module-slot"; // sensible default
        var mode = "hard";
        var dummy = "marker";
        var dirOverwrite = 1; // 0=never,1=ask,2=always
        ...
        if (Directory.Exists(outFull))
        {
            ...
            if (nonEmpty)
                switch (dirOverwrite)
                {
                    case 0: // never
                        return Fail(
                            $"Target dir exists and is not empty: '{outFull}'. Use --overwrite ask|always to proceed.");
                    case 1: // ask
                        if (Console.IsInputRedirected)
                            return Fail(
                                $"Target dir exists and is not empty: '{outFull}'. Refusing to overwrite in non-interactive mode (use --overwrite always).");
                        Console.Error.Write(
                            $"Target dir exists and is not empty: '{outFull}'. Proceed and overwrite? [y/N] ");
                        var ans = Console.ReadLine();
                        if (string.IsNullOrWhiteSpace(ans) ||
                            !(ans.Equals("y", StringComparison.OrdinalIgnoreCase) ||
                              ans.Equals("yes", StringComparison.OrdinalIgnoreCase)))
        ...
        for (var i = 0; i < dsa.Count; i++)
        {
            ...
            if (IsClearlyNonexistent(e))
            {
                ...
                Console.Error.WriteLine(
                    $"warn: objects/idx: skipping non-existent entry {i:D5} (name='{e.Name ?? ""}', size={e.Size}, offset={e.Offset}, date={e.mDosDateTime})");
                continue;
            }
        ...
        if (views.Contains("index"))
        {
            ...
            for (var i = 0; i < dsa.Count; i++)
            {
                ...
                {
                    Console.Error.WriteLine(
                        $"warn: by-index: skipping non-existent entry {i:D5} (name='{e.Name ?? ""}', size={e.Size}, offset={e.Offset}, date={e.mDosDateTime})");
        ...
        if (views.Contains("module-slot"))
        {
            ...
            for (var mi = 0; mi < dsa.mModuleCount; mi++)
            {
                ...
                for (var s = 0; s < m.Size; s++)
                {
                    ...
                    {
                        Console.Error.WriteLine(
                            $"warn: module-slot: module '{m.Name ?? ""}' (#{mi}) slot {s:D5} maps to invalid file index {idx} (file_count={dsa.Count}); skipping.");
                        continue;
                    }
                    ...
                    {
                        Console.Error.WriteLine(
                            $"warn: module-slot: module '{m.Name ?? ""}' (#{mi}) slot {s:D5} maps to nonexistent entry {idx:D5}; skipping.");
                        continue;
                    }
        ...
        if (views.Contains("module-name"))
        {
            ...
            for (var mi = 0; mi < dsa.mModuleCount; mi++)
            {
                ...
                for (var s = 0; s < m.Size; s++)
                {
                    ...
                    {
                        Console.Error.WriteLine(
                            $"warn: module-name: module '{m.Name ?? ""}' (#{mi}) slot {s:D5} maps to invalid file index {idx} (file_count={dsa.Count}); skipping.");
                        continue;
                    }
                    ...
                    {
                        Console.Error.WriteLine(
                            $"warn: module-name: module '{m.Name ?? ""}' (#{mi}) slot {s:D5} maps to nonexistent entry {idx:D5}; skipping.");
                        continue;
                    }
        ...
        if (views.Contains("name"))
        {
            ...
            for (var mi = 0; mi < dsa.mModuleCount; mi++)
            {
                ...
                for (var s = 0; s < m.Size; s++)
                {
                    ...
                    {
                        Console.Error.WriteLine(
                            $"warn: by-name: module '{m.Name ?? ""}' (#{mi}) slot {s:D5} maps to invalid file index {idx} (file_count={dsa.Count}); skipping.");
                        continue;
                    }
                    ...
                    {
                        Console.Error.WriteLine(
                            $"warn: by-name: module '{m.Name ?? ""}' (#{mi}) slot {s:D5} maps to nonexistent entry {idx:D5}; skipping.");
                        continue;
                    }
        ...
        Console.Error.WriteLine(
            $"summary: objects written={objectsWritten}, empty entries skipped={objectsSkippedEmpty}");
        return 0;
    }
    ...
    private static bool CanCreateSymlinksWindows()
    {
        ...
        var tempRoot = Path.Combine(Path.GetTempPath(), "rivax_symlink_probe_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        var target = Path.Combine(tempRoot, "target.tmp");
        var link = Path.Combine(tempRoot, "link.tmp");
    ...
    private static HashSet<string> ParseViews(string v)
    {
        if (string.Equals(v, "all", CI))
            return new HashSet<string>(["index", "module-slot", "module-name", "name"],
    ...
    private static void LinkToObject(string destPath, string objectsIdxDir, int entryIndex, string mode)
    {
        ...
        try
        {
            if (File.Exists(destPath))
            {
                ...
                {
                    /* if delete fails, fallback paths may still error */
                }
            }
        }
        catch
        ...
        {
            Console.Error.WriteLine(
                $"warn: missing source object for index {entryIndex:D5} at '{src}'; skipping link '{destPath}'");
            return;
        }
    ...
    private static bool IsClearlyNonexistent(Dsa3Entry e)
    {
        ...
        var noName = string.IsNullOrEmpty(e.Name) || string.IsNullOrWhiteSpace(e.Name);
        var rawZero = e.NameRaw13 != null && e.NameRaw13.All(b => b == 0);
        var zeroMeta = e is { Size: 0, Offset: 0, mDosDateTime: 0 };
    ...
    private static int UnixLink(string existing, string @new)
    {
    ...
    {
        if (args.Length < 1)
            return Fail("Usage: rivaxtract list <archive> [--pattern <glob>] [--modules-only] [--csv <path>]");
        var archive = args[0];
        ...
        if (modulesOnly)
            ...
            foreach (var (mod, _, entryIndex) in EnumerateVisibleSlots(dsa))
            {
        ...
        if (!string.IsNullOrEmpty(csvPath)) dsa.PrintAllIndices(true, false, csvPath);
    ...
    private static int CmdExtract(string[] args)
    {
        if (args.Length < 1)
            return Fail(
                "Usage: rivaxtract extract <archive> -o <outdir> [--pattern <glob>] [--module-dirs] [--overwrite ask|never|always]");
        var archive = args[0];
        var outdir = "";
        var pattern = "*";
        var moduleDirs = false;
        var overwriteMode = 1; // 0=never,1=ask,2=always
            ...
            if (!moduleDirs)
            {
                ...
                if (File.Exists(candidate))
                    throw new IOException(
                        $"Filename collision for '{e.Name}'. Use --module-dirs to keep files separated by module.");
            }
    ...
    private static int CmdRead(string[] args)
    {
        ...
        if (args.Length < 3)
            return Fail(
                "Usage: rivaxtract read <archive> <module> <filename> [-o <path>] [--offset N] [--length N] [--info]");
        ...
        var (mod, entry, moduleSlot, index) = FindEntry(dsa, module, filename)
                                              ?? throw new FileNotFoundException(
                                                  $"Entry '{module}{PathDelim()}{filename}' not found.");
    ...
    private static int CmdModify(string[] args)
    {
        ...
        if (args.Length < 1)
            return Fail(
                "Usage: rivaxtract modify <archive> [--out <new-archive>] [--set MOD/FILE=path]... [--from-dir <root>] [--map <csv|json>] [--strict] [--dry-run] [--inplace] [--touch]");
        ...
        foreach (var (_, _, _, oldSize, newSize) in plan)
            if (newSize != oldSize)
            {
                allSameSize = false;
                break;
        ...
        if (dryRun)
        {
            ...
            foreach (var p in plan)
                Console.Error.WriteLine(
                    $"  {p.module}{PathDelim()}{p.filename}  idx={p.index:D5}  size {p.oldSize} -> {p.newSize}");
        ...
        {
            var target = string.IsNullOrEmpty(outArc) ? archive : outArc;

            if (!string.IsNullOrEmpty(outArc))
                // Copy archive then patch the copy
                File.Copy(archive, outArc, true);
            ...
            foreach (var kv in replByIndex)
            {
                ...
                var bytes = kv.Value;
                var e = (Dsa3Entry)dsa.Entries[idx];
                var pos = (long)dsa.HeaderSize + e.Offset;
            ...
            Console.Error.WriteLine(
                $"patched {(string.IsNullOrEmpty(outArc) ? "in-place" : $"→ '{outArc}'")} ({replByIndex.Count} entr{(replByIndex.Count == 1 ? "y" : "ies")})");
            return 0;
        }
        ...
        if (touch)
        {
            ...
            var packed = DosDateTime.Encode(now);
            foreach (var kv in replByIndex)
            {
        ...
        Console.Error.WriteLine(
            $"modified {replByIndex.Count} entr{(replByIndex.Count == 1 ? "y" : "ies")} → '{outArc}' (indices/slots/dummies preserved)");
        return 0;
    }
    ...
    private static void MergeReplacementsFromMap(Dictionary<string, byte[]> dst, string mapFile)
    {
        ...
        if (ext is ".csv")
        {
            ...
            var iMod = Array.FindIndex(header, h => h.Equals("Module", StringComparison.InvariantCultureIgnoreCase));
            var iFile = Array.FindIndex(header, h => h.Equals("Filename", StringComparison.InvariantCultureIgnoreCase));
            var iPath = Array.FindIndex(header, h => h.Equals("Path", StringComparison.InvariantCultureIgnoreCase));
        ...
        else if (ext is ".json")
        {
            ...
            var items = JsonSerializer.Deserialize<List<MapItem>>(txt, jopts)
                        ?? [];
    ...
    private static int CmdBuildFromJson(string[] args)
    {
        ...
        var opts = new JsonSerializerOptions { TypeInfoResolver = RivaJsonContext.Default };
        var root = JsonSerializer.Deserialize<JsonRoot>(File.ReadAllText(jsonPath), opts)
                   ?? throw new InvalidOperationException("Invalid JSON.");

        ValidateJsonStructure(root);

        // Construct a Dsa3 model exactly from JSON tables/fields
        var (_, dsa) = NewDsaFromJson(root, outArc);

        // Load payloads by entry index from objects/idx
        var idxDir = ResolveIdxDir(objectsRoot);
        var entryData = LoadEntryDataByIndex(root, idxDir, false); // presence check; sizes validated below

        // Check for size mismatches vs JSON
        var mism = new List<(int index, uint jsonSize, int bytes)>();
        for (var i = 0; i < root.Entries!.Count; i++)
        {
            var jsz = root.Entries[i].SizeU32;
            var bsz = entryData[i].Length;
        ...
        if (mism.Count > 0 && !strict)
        {
            ...
            ofs2.Close();
            Console.Error.WriteLine(
                $"built (repacked) '{outArc}' from JSON '{jsonPath}' and objects '{idxDir}' (sizes changed for {mism.Count} entries)");
            return 0;
        }
        ...
        if (mism.Count > 0 && strict)
        {
            foreach (var m in mism)
                Console.Error.WriteLine(
                    $"error: size mismatch for {m.index:D5}.dat: JSON={m.jsonSize}, bytes={m.bytes}");
            return 1;
        }
        ...
        using (var ofs = new Ofstream(outArc, (int)total))
        {
            try
            {
                dsa.WriteVerbatim(ofs, entryData);
            }
    ...
    private static (Config cfg, Dsa3 dsa) NewDsaFromJson(JsonRoot root, string archiveName)
    {
        ...
        for (var i = 0; i < root.Entries.Count; i++)
        {
            ...
            if (!string.IsNullOrEmpty(je.RawFilenameHex))
            {
                ...
                if (raw.Length != 13)
                    throw new InvalidOperationException(
                        $"entry {i} raw_filename_hex must be 13 bytes (26 hex chars), got {raw.Length} bytes");
                e.NameRaw13 = raw;
            }
        ...
        uint maxMapEnd = 0;
        for (var mi = 0; mi < root.Modules.Count; mi++)
        {
            ...
            if (!string.IsNullOrEmpty(jm.RawFilenameHex))
            {
                ...
                if (raw.Length != 14)
                    throw new InvalidOperationException(
                        $"module {mi} raw_filename_hex must be 14 bytes (28 hex chars), got {raw.Length} bytes");
                m.NameRaw14 = raw;
            }
    ...
    private static byte[] HexToBytes16(string hex)
    {
        ...
        return hex.Length != 32 ? throw new ArgumentException("header.unknown16_hex must be exactly 32 hex characters.") : HexToBytes(hex);
    }
    ...
    private static IList<byte[]> LoadEntryDataByIndex(JsonRoot root, string idxDir, bool strictSizes = true)
    {
        ...
        for (var i = 0; i < root.Entries.Count; i++)
        {
            ...
            if (strictSizes && bytes.Length != expected)
                throw new InvalidOperationException(
                    $"Size mismatch for {Path.GetFileName(p)}: bytes={bytes.Length}, JSON={expected}");
            list.Add(bytes);
        }
    ...
    private static uint ComputeExactArchiveSize(JsonRoot root)
    {
        ...
        // Module table end
        var modulesEnd = h.ModuleTableOffsetU32 + (ulong)root.Modules.Count * ModRec;

        // File table end
        var filesEnd = h.FileTableOffsetU32 + (ulong)root.Entries.Count * FileRec;

        // Modmap size: max over (modmap_start + slot_count) across modules
        uint modMapSize = 0;
        foreach (var m in root.Modules)
        {
            var slots = (uint)(m.Slots?.Count ?? 0);
            var end = checked(m.ModMapStartU32 + slots);
            if (end > modMapSize) modMapSize = end;
        }

        var modmapEnd = h.ModMapOffsetU32 + modMapSize * 2UL; // uint16 entries

        // Data region end: data_offset + max(entry.offset + size)
        var dataEnd = (ulong)h.DataOffsetU32;
        foreach (var e in root.Entries)
        {
            var end = h.DataOffsetU32 + (ulong)e.OffsetU32 + e.SizeU32;
            if (end > dataEnd) dataEnd = end;
        }

        // Final file size must cover all four regions
        var maxEnd = modulesEnd;
    ...
    private static void ValidateJsonStructure(JsonRoot root)
    {
        ...
        if (root.Entries.Count != h.FileCountU16)
            throw new InvalidOperationException(
                $"entries length ({root.Entries.Count}) != header.file_count_u16 ({h.FileCountU16})");

        if (root.Modules.Count != h.ModuleCountU16)
            throw new InvalidOperationException(
                $"modules length ({root.Modules.Count}) != header.module_count_u16 ({h.ModuleCountU16})");

        for (var mi = 0; mi < root.Modules.Count; mi++)
        {
            var m = root.Modules[mi];
            var slotsLen = m.Slots?.Count ?? 0;
            if ((uint)slotsLen != m.SizeSlotsU32)
                throw new InvalidOperationException(
                    $"module {mi} '{m.Name}' slots length ({slotsLen}) != size_slots_u32 ({m.SizeSlotsU32})");
            ...
            if (m.Slots == null) continue;
            for (var s = 0; s < m.Slots.Count; s++)
            {
                uint v = m.Slots[s];
                if (v == 0xFFFF) continue;
                if (v >= h.FileCountU16)
                    throw new InvalidOperationException(
                        $"module {mi} slot {s}: modmap index {v} >= file_count ({h.FileCountU16})");
            }
        }
    ...
    private static void ApplyPattern(Dsa3 dsa, string pattern)
    {
        ...
        for (var mi = 0; mi < dsa.mModuleCount; mi++)
        {
            ...
            for (var slot = 0; slot < mod.Size; slot++)
            {
                var mapIdx = (int)(mod.Offset / 2 + slot);
    ...
    private static (Dsa3ModuleEntry mod, Dsa3Entry entry, int moduleSlot, int index)? FindEntry(Dsa3 dsa,
    ...
    private static int PrintTopHelp()
    {
        Console.WriteLine("""
                          ...
                          Examples:
                            rivaxtract export-json RIVA.ALF --out RIVA.structure.json --pretty
                            rivaxtract export-views RIVA.ALF --out outdir --views index,module-slot
                            rivaxtract build RIVA.structure.json --objects outdir --out RIVA_rebuilt.ALF
