...
/*
DSA3 ALF file format (as used here)

All integers are little‑endian. Strings are ASCII, null‑terminated.

Header (offsets from start of file):
| Offset | Meaning                                                        |
|  0–3   | Signature "ALF " (0x41 0x4C 0x46 0x20)                         |
|  4–7   | Version (u32), e.g., 0x00000100                                |
|  8–9   | File table size in entries (u16)                               |
| 10–13  | File table offset (u32)                                        |
| 14–15  | File count (u16) — actual number of file entries               |
| 16–19  | Data offset (u32) — start of payload area (“header size”)      |
| 20–21  | Module table size in entries (u16)                             |
| 22–25  | Module table offset (u32)                                      |
| 26–27  | Module count (u16)                                             |
| 28–31  | Module map offset (u32)                                        |
| 32–47  | Reserved/unknown (16 bytes), preserved verbatim, always 0      |

File table entry (28 bytes each):
| Offset | Meaning                                                        |
|  0–12  | Filename (13 bytes, ASCII, null‑terminated)                    |
|   13   | Unknown (u8), appears to be always either 0 or 176 (0xB0)      |
| 14–17  | File length (u32)                                              |
| 18–21  | FAT/DOS date/time (u32, local)                                 |
| 22–23  | Unknown (u16), appears to be always 0                          |
| 24–27  | Offset (u32), relative to Data offset (not absolute)           |

Module table entry (28 bytes each):
| Offset | Meaning                                                        |
|  0–13  | Module name (14 bytes, ASCII, null‑terminated)                 |
| 14–17  | Number of slots in this module (u32)                           |
| 18–21  | FAT/DOS date/time (u32, local)                                 |
| 22–23  | Unknown (u16), appears to be always 0                          |
| 24–27  | Start in module map × 2 (u32). Divide by 2 for first slot idx. |

Module map:
- Located at “Module map offset”; entries are uint16.
- Logical length = max over modules of (module.offset/2 + module.size).
- Each entry is a file table index; 0xFFFF marks a dummy/empty slot.

Notes:
- Entry offsets are relative to the Data offset (header size), not the file start.
*/
...
public class Dsa3ModuleEntry : IHeaderEntry
{
    ...
    public List<ushort?> mEntryIndices = [];
...
public class Dsa3
{
    ...
    public List<IHeaderEntry> Entries { get; set; } = [];
    ...
    public void BuildEntryDataFromIndex(IList<byte[]> entryDataByIndex)
    {
        ...
        for (var mi = 0; mi < mModuleCount; mi++)
        {
            var mod = mModules[mi];
    ...
    public uint ComputePackedSize(IList<byte[]> entryDataByIndex)
    {
        ...
        var header =
            48u + // fixed header
            mModuleCount * 28u + // module table
            Count * 28u + // file table
            mModMapSize * 2u; // modmap (uint16 each)
    ...
    public void RepackTo(Ostream outStream, IList<byte[]> entryDataByIndex)
    {
        ...
        mModMapOffset = mFiletableOffset + Count * 28u;
        mDataOffset = mModMapOffset + mModMapSize * 2u;
    ...
    public void PrintAllIndices(bool includeDummies = false, bool oneBased = false, string file = "")
    {
        ...
        try
        {
            ...
            for (var mi = 0; mi < mModuleCount; mi++)
            {
                ...
                for (var slot = 0; slot < module.Size; slot++)
                {
                    var mapIdx = (int)(module.Offset / 2 + slot);
                    var entryIndex = (int)mModMap[mapIdx];

                    if (entryIndex == 0xFFFF)
                    {
                        if (includeDummies)
                            writer.WriteLine($"{mi}, {module.Name}, {(oneBased ? slot + 1 : slot)}, - , - ");
                    ...
                    var filename = entry.Name; // or: $"{module.Name}{mConfig.mPathDelim}{entry.Name}" if you prefer
                    var slotOut = oneBased ? slot + 1 : slot;
                    var idxOut = oneBased ? entryIndex + 1 : entryIndex;

                    writer.WriteLine($"{mi}, {module.Name}, {slotOut}, {idxOut}, {filename}");
                }
            }
        }
        finally
    ...
    public void Write(Ostream strm)
    {
        ...
        foreach (var modMap in mModMap)
            strm.Write16((ushort)modMap);

        for (var i = 0; i < mModules.Count; i++)
        {
            var module = mModules[i];

            for (var j = 0; j < mModules[i].Size; j++)
            {
                var entryIndex = (int)mModMap[(int)mModules[i].Offset / 2 + j];
                if (entryIndex == 0xFFFF) continue;
    ...
    public bool Read(Istream strm)
    {
        ...
        {
            var module = new Dsa3ModuleEntry();
            module.Read(strm);
            mModules.Add(module);
            if (module.Offset / 2 + module.Size > mModMapSize) mModMapSize = module.Offset / 2 + module.Size;
        }

        strm.Seekg(mFiletableOffset);
        for (uint i = 0; i < Count; i++)
        {
            var entry = new Dsa3Entry();
            entry.Read(strm);
            Entries.Add(entry);
        }

        strm.Seekg(mModMapOffset);
        for (uint i = 0; i < mModMapSize; i++) mModMap.Add(strm.Read16());

        for (var i = 0; i < mModuleCount; i++)
        for (var j = 0; j < mModules[i].Size; j++)
        {
            var entryIndex = (ushort)mModMap[(int)mModules[i].Offset / 2 + j];
            if (entryIndex == 0xFFFF)
            {
            ...
            if (entryIndex < Entries.Count)
            {
                ...
                var entry = (Dsa3Entry)Entries[entryIndex];
    ...
    public IHeaderEntry NewHeaderEntry()
    {
        return new Dsa3Entry();
    }

    private class DebugTextWriter : TextWriter
    {
        public static readonly DebugTextWriter Instance = new();
...
public class Istream : IDisposable
{
    ...
    {
        var result = (ushort)(mContent[mPosition] | (mContent[mPosition + 1] << 8));
        mPosition += 2;
        return result;
    }
    ...
    public void Seekg(uint position)
    {
...
internal class Ifstream : Istream
{
    public Ifstream(string path)
    {
        ...
        {
            var n = fs.Read(mContent, read, mFilesize - read);
            if (n == 0) break;
            read += n;
        }
    ...
    public bool Eof()
    {
        return mPosition >= (mContent?.Length ?? 0);
    }
...
internal class Ofstream : Ostream
{
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        try
        {
            if (!disposing || _fs is null) return;
